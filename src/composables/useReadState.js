...
const THREAD_KEY = "readThreadIds"
...
const readThreadIds = ref(loadSet(THREAD_KEY))
...
function markThreadRead(id) {
  if (!id || readThreadIds.value.has(id)) return
  const next = new Set(readThreadIds.value)
  next.add(id)
  readThreadIds.value = next
  persist(THREAD_KEY, next)
}
...
function isThreadRead(thread) {
  return readThreadIds.value.has(thread.id)
}
