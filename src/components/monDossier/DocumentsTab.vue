      ...
      <v-card class="card-shadow pa-6" :class="{ 'rounded-15': !$vuetify.display.mobile }">
        ...
        <v-row>
          <v-col v-for="doc in REQUIRED_DOCS" :key="doc.key" cols="12" sm="6" md="4">
            <div class="doc-card" :class="{ 'doc-card--uploaded': documents[doc.key], 'doc-card--missing': !documents[doc.key] }"
              ...
              <!-- Status badge -->
              <div class="doc-card-badge" :class="documents[doc.key] ? 'doc-card-badge--ok' : 'doc-card-badge--todo'">
                <v-icon :icon="documents[doc.key] ? mdiCheck : mdiPlus" size="14" />
              </div>

              <!-- Icon -->
              <div class="doc-card-icon" :class="documents[doc.key] ? 'doc-card-icon--ok' : 'doc-card-icon--todo'">
